...
// payment data beyond the session id, and never trust the request without a valid
// signature.
...
function extractRequestText(session) {
  ...
  return field?.text?.value?.trim() || "(no description provided)";
}
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      return new Response(JSON.stringify({ queued: issue.number }), {
